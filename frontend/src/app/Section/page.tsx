...
export default function Section() {
  ...
  const deleteSection = async (id: SectionsData) => {
    ...
    if (response.status <= 205)
      return toast.success('Sessão deletada com sucesso!');
          ...
          <Dialog>
            <form onSubmit={handleSubmit(onSubmit)}>
              <DialogTrigger asChild>
                <Button
                  className='bg-blue-900 hover:bg-blue-800 rounded-lg hover:text-white cursor-pointer text-white mb-2 ml-6 mt-6 font-bold'
                  variant='outline'
                >
                  Nova Sessão
                </Button>
            ...
            <table className='w-full text-sm text-left rtl:text-right text-gray-500'>
              ...
              <tbody>
                {Sections?.map((section) => (
                  <tr
                    key={section.id}
                    className='bg-gray-300 border-b uppercase text-black border-gray-200'
                  >
                    <td
                      scope='row'
                      className='px-6 py-4'
                    ...
                    <td
                      scope='row'
                      className='px-6 py-4 flex flex-row gap-2'
                    >
                      <span className='text-[1.5rem] cursor-pointer text-blue-400 hover:text-blue-600'>
                        <CiEdit />
                      </span>
                      <span
                        className='text-[1.5rem] cursor-pointer text-red-500 hover:text-red-700'
